...
async function generateQuestionsForCategory(category, subDomain, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      let triviaCategory = await TriviaCategory.findOne({ category, subDomain: subDomain });
      ...
      console.log(
        `[GENERATOR] SUCCESS: Added ${addedCount} questions (${duplicateCount} duplicates skipped) for "${category}/${subDomain}"`
      );
